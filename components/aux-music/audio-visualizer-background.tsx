...
class PrefabBufferGeometry extends THREE.BufferGeometry {
  prefabGeometry: THREE.BufferGeometry
  prefabCount: number
...
export function AudioVisualizerBackground() {
  ...
  const lightsRef = useRef<{ light: THREE.PointLight; light2: THREE.DirectionalLight; light3: THREE.DirectionalLight } | null>(null)
  const analyserRef = useRef<SpectrumAnalyzer | null>(null)
  const audioElementRef = useRef<HTMLAudioElement | null>(null)
  const animationFrameRef = useRef<number | null>(null)
  const audioStartedRef = useRef(false)

  const SHADOW_COLOR = 0x13091b
  ...
  useEffect(() => {
    ...
    // Lights
    const light = new THREE.PointLight(0xffffff, 1, 1200, 2)
    light.position.set(0, 0, 0)
    scene.add(light)

    const light2 = new THREE.DirectionalLight(0xff311f, 1)
    light2.position.set(0, 1, 1)
    scene.add(light2)

    const light3 = new THREE.DirectionalLight(0xffd633, 0.25)
    light3.position.set(1, 1, 0)
    scene.add(light3)

    lightsRef.current = { light, light2, light3 }
    ...
    const tick = () => {
      ...
      // Apply parallax camera position
